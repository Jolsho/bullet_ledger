...
BulletDB* lmdb_open(const char* path, size_t map_size) {
    BulletDB* handle = new BulletDB();
    handle->keys.reserve(10);
    ...
    handle->l2.reserve(10);
    handle->l3.reserve(10);
...
int lmdb_put(BulletDB* handle, const void* key_data, size_t key_size,
...
int lmdb_get(BulletDB* handle, const void* key_data, size_t key_size,
...
void* mutable_get(BulletDB* handle, const void* key, size_t key_size, size_t value_size) {
    ...
    int rc = mdb_put(handle->txn, handle->dbi, &k, &v, MDB_RESERVE);
...
std::vector<uint64_t> flatten_sort_l2(BulletDB* handle) {
    ...
    keys.reserve(handle->l2.size() + handle->l3.size());

    // copy l2
    for (const auto& [key, value] : handle->l2)
        keys.push_back(key);

    // copy l3
    for (const auto& [key, value] : handle->l3)
        keys.push_back(key);

    auto middle = keys.begin() + handle->l2.size();
