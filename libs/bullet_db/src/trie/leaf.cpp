...
Leaf::Leaf(Gadgets_ptr gadgets, const NodeId* id, const ByteSlice* buff) : 
    id_(id),
...
void Leaf::set_path(const Hash* key, uint16_t block_id) {
...
std::optional<size_t> Leaf::matching_path(const Hash* key, int i) {
...
int Leaf::prune(const uint16_t block_id) {
...
int Leaf::justify(const uint16_t block_id) {
    NodeId child_id_; 
