...
public:
    ...
    void set_path(const Hash* key, int i) { 
        std::memcpy(path_.h, &key->h[i], sizeof(key->h) - i);
    }

    const Commitment* get_commitment() const override { 
        return &commit_; 
    }
    void set_commitment(const Commitment &c) override { 
        commit_ = c; 
    }

    const byte get_type() const override { 
        return BRANCH; 
    }
...
inline std::shared_ptr<Branch> create_branch(Gadgets_ptr gadgets, const NodeId* id, const ByteSlice* buff) {
