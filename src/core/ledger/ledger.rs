...
impl Ledger { 
    ...
    pub(super) fn load_node(&mut self, id: &NodeID) -> Option<Node> {
    ...
    pub(super) fn cache_node(&mut self, id: u64, node: Node) {
    ...
    pub(super) fn new_cached_leaf(&mut self, id: u64) -> Rc<RefCell<Leaf>> {
        let l = Leaf::new(Some(id.to_le_bytes()));
        self.cache_node(id, Node::Leaf(l.clone()));
        l
    }

    pub(super) fn new_cached_branch(&mut self, id: u64) -> Rc<RefCell<BranchNode>> {
        let b = BranchNode::new(Some(id.to_le_bytes()));
        self.cache_node(id, Node::Branch(b.clone()));
        b
    }

    pub(super) fn new_cached_ext(&mut self, id: u64) -> Rc<RefCell<ExtNode>> {
